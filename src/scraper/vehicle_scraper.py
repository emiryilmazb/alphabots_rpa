...
class VehicleScraper:
    ...
    async def collect_vehicle_entries(self, dealer_url: str) -> list[dict[str, str]]:
        ...
        for category_index, category_value in enumerate(category_values):
            ...
            category_report = {
                ...
                "label": metadata.get("source_category_label")
                or VEHICLE_CATEGORY_LABELS.get(category_value or "", "base"),
            ...
            for entry in category_entries:
                ...
                if (
                    ...
                    and len(entries_by_key) >= self.config.max_cars_per_vendor
                ):
                    break
                ...
                if remaining_categories:
                    ...
                    for remaining in remaining_categories:
                        ...
                        self.last_category_report.append(
                            {
                                "category": remaining,
                                "label": remaining_metadata.get("source_category_label")
                                or VEHICLE_CATEGORY_LABELS.get(remaining, ""),
                                "count": remaining_metadata.get(
    ...
    async def _collect_entries_from_loaded_inventory(
        ...
        while True:
            ...
            for url, summary in structured_summaries.items():
                self._apply_category_fallback(
                    summary, category_value, category_metadata
                )
            ...
            if page_index > 1000:
                logger.warning(
                    "Stopping listing pagination after 1000 pages for %s", dealer_url
                )
                break
            await self.browser.polite_delay()
    ...
    async def _category_sequence_from_current_page(self) -> list[str | None]:
        ...
        try:
            ...
            html = ""
        discovered_options = parse_vehicle_category_options(
            html, require_positive_count=True
        )
        self.category_metadata = {
            str(option["value"]): {
                ...
                "source_category_label": str(
                    option.get("label")
                    or VEHICLE_CATEGORY_LABELS.get(str(option["value"]), "")
                ),
        ...
        discovered = [
            str(option["value"]) for option in discovered_options if option.get("value")
        ]
    ...
    def _entry_key(entry: dict[str, str]) -> str:
        return normalize_vehicle_url(entry.get("Vehicle_URL", "")) or "|".join(
    ...
    def _merge_entry(
        ...
        for key, value in incoming.items():
            if value and not merged.get(key):
                merged[key] = value
        if sum(bool(clean_text(v)) for v in incoming.values()) > sum(
            bool(clean_text(v)) for v in existing.values()
        ):
            for key, value in incoming.items():
                if value:
                    merged[key] = value
    ...
    def _apply_category_fallback(
        ...
        if category_value:
            ...
            if category_metadata.get("source_category_count") is not None:
                summary.setdefault(
                    "source_category_count",
                    str(category_metadata["source_category_count"]),
                )
            summary.setdefault("source_category_url", source_url)
            if label and VehicleScraper._should_replace_vehicle_type(
                summary.get("Fahrzeugtyp", "")
            ):
    ...
    def _should_replace_vehicle_type(value: str) -> bool:
        ...
        return lowered in condition_or_stat or bool(
            re.search(
                r"\b(?:EZ\s*)?\d{2}/\d{4}\b|\b\d[\d. ]*\s*km\b|\b\d{1,4}\s*kW\b",
                text,
                re.I,
            )
        )
    ...
    async def _legacy_collect_vehicle_urls(self, dealer_url: str) -> list[str]:
        ...
        while True:
            ...
            if (
                self.config.max_cars_per_vendor > 0
                and len(all_urls) >= self.config.max_cars_per_vendor
            ):
    ...
    def _synthetic_vehicle_url(
        ...
        title = " ".join(
            part
            for part in [summary.get("Markes", ""), summary.get("Models", "")]
            if part
        ).strip()
    ...
    async def _extract_listing_summaries_from_dom(
        ...
        for row in rows or []:
            ...
            summary = {
                ...
                "Markes": brand,
                "Models": model,
                "Preis": clean_text(row.get("price", "")).splitlines()[0]
                if row.get("price")
                else "",
                "Kilometerstand": self._match_text(details, r"\b\d[\d. ]*\s*km\b"),
                "Erstzulassung": self._match_text(
                    details, r"\b(?:EZ\s*)?\d{2}/\d{4}\b"
                ).replace("EZ ", ""),
                "Leistung": self._match_text(
                    details, r"\b\d{1,4}\s*kW\s*\(\s*\d{1,4}\s*PS\s*\)|\b\d{1,4}\s*kW\b"
                ),
                "Kraftstoffart": self._match_text(
                    details,
                    r"\b(?:Benzin|Diesel|Elektro|Hybrid|Erdgas|Autogas|Wasserstoff)\b",
                ),
                "Getriebe": self._match_text(
                    details, r"\b(?:Automatik|Schaltgetriebe|Halbautomatik)\b"
                ),
                **attr_fields,
            }
        ...
        for selector in selectors:
            try:
                ...
                if (
                    ...
                    or await page.locator("[data-testid*='listing']").count() > 0
                ):
                    return True
            except Exception as e:
    ...
    async def scrape_vehicle(
        ...
        if not fetch_result.ok:
            ...
            vehicle["detail_status"] = (
                fetch_result.detail_status
                or fetch_result.classification
                or "fetch_failed"
            )
        ...
        detail_filled_fields = self._merge_detail_fields(
            vehicle, detail_fields, fetch_result.strategy
        )
        target_extracted_count = sum(
            1
            for field in DETAIL_TARGET_FIELDS
            if clean_text(detail_fields.get(field, ""))
        )
        if target_extracted_count:
            self._increment_config_counter(
                "detail_target_fields_extracted_count", target_extracted_count
            )
    ...
    def _merge_detail_fields(
        ...
        for key, value in detail_fields.items():
            ...
            if clean_text(vehicle.get(key, "")) != clean_text(value):
                conflicts[key] = {
                    "listing": clean_text(vehicle.get(key, "")),
        ...
        if source == "uc-popup" and filled:
            self._increment_config_counter(
                "fields_added_by_uc_popup_count", len(filled)
            )
        return filled
