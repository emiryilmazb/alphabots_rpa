...
from src.scraper.parser_modules.normalization import clean_text, normalize_vehicle_url
    ...
    for payload in extract_next_payloads(html):
        for obj in iter_dicts(payload):
            ...
            summary = {
                ...
                "Erstzulassung": _first_match(
                    attrs, r"\b(?:EZ\s*)?\d{2}/\d{4}\b"
                ).replace("EZ ", ""),
                "Leistung": _first_match(
                    attrs, r"\b\d{1,4}\s*kW\s*\(\s*\d{1,4}\s*PS\s*\)|\b\d{1,4}\s*kW\b"
                ),
                "Kraftstoffart": _first_match(
                    attrs,
                    r"\b(?:Benzin|Diesel|Elektro|Hybrid|Erdgas|Autogas|Wasserstoff)\b",
                ),
                "Getriebe": _first_match(
                    attrs, r"\b(?:Automatik|Schaltgetriebe|Halbautomatik)\b"
                ),
                "Fahrzeugzustand": _first_match(
                    attrs,
                    r"\b(?:Gebrauchtfahrzeug|Neufahrzeug|Jahreswagen|Vorführfahrzeug|Tageszulassung)\b",
                ),
                "Schadstoffklasse": _first_match(attrs, r"\bEuro\s*\d[a-zA-Z]?\b"),
                **attr_fields,
            }
            _merge_listing_summary(
                summaries, url, {k: v for k, v in summary.items() if v}
            )
    ...
    *,
    ...
    def add(value: Any, *, label: Any = "", count: Any = None, url: Any = "") -> None:
        ...
        if existing is None:
            existing = {
                "value": text,
                "label": _none_if_placeholder(label)
                or VEHICLE_CATEGORY_LABELS.get(text, text),
                "count": parsed_count,
    ...
    for payload in extract_next_payloads(html):
        for obj in iter_dicts(payload):
            ...
            label = _first_present(
                obj.get("label"), obj.get("name"), obj.get("localized")
            )
...
def _category_sidebar_label(label_elem: Tag, value: str) -> str:
    for elem in label_elem.find_all(True):
        ...
        if "vehicleCategoryChipLabel" in class_text:
            ...
            if text:
                return text
    return _category_label_from_text(
        clean_text(label_elem.get_text(" ", strip=True)), value
    )
...
def _parenthesized_count_from_text(text: str) -> int | None:
    matches = re.findall(
        r"[\(\[]\s*(\d{1,6}(?:[.\s]\d{3})*)\s*[\)\]]", clean_text(text)
    )
    if not matches:
        return None
...
def _category_aliases(value: str) -> list[str]:
    ...
    return [
        alias
        for alias in dict.fromkeys(clean_text(alias) for alias in aliases)
        if alias
    ]
...
def _summary_from_search_result_listing(listing: dict[str, Any]) -> dict[str, str]:
    ...
    summary = {
        ...
        "Hubraum": _first_present(
            attr.get("cc"), attr.get("ccm"), attr.get("displacement")
        ),
...
def _model_text_from_listing(title: str, make: str, listing: dict[str, Any]) -> str:
    ...
    return (
        clean_text(f"{model} {subtitle}")
        if model or subtitle
        else split_vehicle_title(title)[1]
    )
...
def _vehicle_type_from_listing(listing: dict[str, Any], attr: dict[str, Any]) -> str:
    ...
    return (
        VEHICLE_CATEGORY_LABELS.get(vc)
        or _vehicle_body_label(raw_category)
        or raw_category
    )
...
def _financing_from_search_result_listing(
    listing: dict[str, Any], vehicle_price: str
) -> dict[str, str]:
    ...
    localized = (
        offer.get("localized") if isinstance(offer.get("localized"), dict) else {}
    )
    monthly = _first_present(
        localized.get("monthlyInstallment"), offer.get("monthlyInstallment")
    )
    result = {
        "Financing": f"{monthly} mtl."
        if monthly and "€" in monthly and "mtl" not in monthly.lower()
        else monthly,
        "Bank": _first_present(offer.get("bankName")),
        "Darlehensvermittler": _first_present(offer.get("loanBroker")),
        "Fahrzeugpreis": vehicle_price,
        "Anzahlung": _first_present(
            localized.get("downPayment"), offer.get("downPayment")
        ),
        "Jährliche Kilometerleistung": _first_present(
            localized.get("yearlyMileage"), offer.get("yearlyMileage")
        ),
        "Schlussrate": _first_present(
            localized.get("finalInstallment"), offer.get("finalInstallment")
        ),
        ...
        "Gesamtzins": _first_present(
            localized.get("totalInterest"), offer.get("totalInterest")
        ),
        "Gesamtbetrag": _first_present(
            localized.get("totalAmount"), offer.get("totalAmount")
        ),
        "Laufzeit": _credit_term(
            localized.get("creditTerm") or offer.get("creditTerm")
        ),
    }
...
def _choose_attribute_text(strings: list[str]) -> str:
    attributes = [
        ...
        for text in strings
        if "•" in text
        or re.search(r"\b(?:EZ|km|kW|PS|Benzin|Diesel|Automatik)\b", text)
    ]
...
def extract_listing_attribute_fields(text: str) -> dict[str, str]:
    ...
    if re.search(
        r"\b(?:EZ\s*)?\d{2}/\d{4}\b|\b\d[\d. ]*\s*km\b|\b\d{1,4}\s*kW\b", first, re.I
    ):
        return {}
